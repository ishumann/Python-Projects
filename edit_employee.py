...
class Edit_employee(tk.Frame):
    """docstring for Add_empl`oyee"""
    def __init__(self, add_emp_window, id):
        global db, emp_window, emp_id
        global emp_name
        global job_title
        db = Data_management()
        listdata = db.fetchdata(id)
        emp_id = id
        emp_window = add_emp_window
        tk.Frame.__init__(self, add_emp_window)
        ...
        emp_name.insert(0, listdata[0][0])
        job_title.insert(0, listdata[0][1])

        savebtn=Button(add_emp_window, text="Save", width=10, background = '#5c4fa1', foreground = "white",command= lambda: update())
...
def update():
    name= emp_name.get()
    title= job_title.get()
    db.updatedata(emp_id ,name, title)
